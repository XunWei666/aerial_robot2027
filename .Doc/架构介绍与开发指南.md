...
- **开发方式**：
  ...
  本框架使用 stm32cubemx 生成外设初始化代码，当前只支持 CMake 构建系统，推荐使用 Ninja 生成器，使用 arm gnu 工具链和 arm-none-eabi-gcc 编译。

  > 迁移说明：本工程从此只支持 CMake，新增多文件时无需手工维护旧构建脚本；CubeMX 再生成后如产生旧构建入口，不纳入本仓库维护。
...
│  LICENSE # 开源协议文件
|  CMakeLists.txt   # CMake 构建规则，本工程唯一受支持的构建入口
