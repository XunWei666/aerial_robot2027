...
typedef enum
{
    MOTOR_FEEDBACK_RPM = 0,
    MOTOR_FEEDBACK_APS
} Feedback_Speed_Unit_e;
...
typedef struct
{
    Closeloop_Type_e outer_loop_type_temp;         // 这一次的最外层的闭环
    Closeloop_Type_e outer_loop_type_last;         // 上一次的最外层的闭环(做边缘检测)
    ...
    Feedback_Source_e last_angle_feedback_source;  // 上次的角度反馈类型
    Feedback_Source_e speed_feedback_source;       // 速度反馈类型
    Feedback_Source_e last_speed_feedback_source;  // 上次速度反馈类型
    Feedfoward_Type_e feedforward_flag;            // 前馈标志
    Feedback_Speed_Unit_e speed_unit_type;
