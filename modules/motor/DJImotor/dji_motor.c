...
static void DJIMotorPIDClear(DJIMotorInstance *instance)
{
    instance->motor_controller.angle_PID.Pout = 0;
    instance->motor_controller.speed_PID.Pout = 0;
    instance->motor_controller.current_PID.Pout = 0;
    instance->motor_controller.angle_PID.Iout = 0;
    instance->motor_controller.speed_PID.Iout = 0;
    instance->motor_controller.current_PID.Iout = 0;
    instance->motor_controller.angle_PID.Dout = 0;
    instance->motor_controller.speed_PID.Dout = 0;
    instance->motor_controller.current_PID.Dout = 0;
    instance->motor_controller.angle_PID.Output = 0;
    instance->motor_controller.speed_PID.Output = 0;
    instance->motor_controller.current_PID.Output = 0;
}
...
{
    motor->motor_settings.outer_loop_type_last = motor->motor_settings.outer_loop_type_temp;
    motor->motor_settings.outer_loop_type_temp = outer_loop;
}
...
void DJIMotorControl()
{
    ...
    { // 减小访存开销,先保存指针引用
        ...
        if (motor_setting->motor_reverse_flag == MOTOR_DIRECTION_REVERSE && motor->motor_type != GM6020)
            pid_ref *= -1; // 设置反转

        if (motor->motor_settings.outer_loop_type_temp != motor->motor_settings.outer_loop_type_last)
        {
            DJIMotorPIDClear(motor);
        }
        // pid_ref会顺次通过被启用的闭环充当数据的载体
        // 计算位置环,只有启用位置环且外层闭环为位置时会计算速度环输出
        if ((motor_setting->close_loop_type & ANGLE_LOOP) && motor_setting->outer_loop_type_temp == ANGLE_LOOP)
        {
            if (motor_setting->angle_feedback_source == IMU_FEED)
        ...
        if ((motor_setting->close_loop_type & SPEED_LOOP) && (motor_setting->outer_loop_type_temp & (ANGLE_LOOP | SPEED_LOOP)))
        {
            if (motor_setting->feedforward_flag & SPEED_FEEDFORWARD)
                pid_ref += *motor_controller->speed_feedforward_ptr;

            if (motor_setting->speed_feedback_source == IMU_FEED)
                pid_measure = *motor_controller->other_speed_feedback_ptr;
            else // MOTOR_FEED
            {
                if (motor_setting->speed_unit_type == MOTOR_FEEDBACK_RPM)
        ...
        // 获取最终输出
        if (motor->motor_type == GM6020 && motor->motor_settings.motor_reverse_flag == MOTOR_DIRECTION_REVERSE)
            pid_ref = -pid_ref;
        set = (int16_t)pid_ref;
        // uint8_t len = snprintf(data, 64, "%d,%d\n", set, measure->real_current);
